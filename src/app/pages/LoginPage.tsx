    ...
    <div className="min-h-screen flex items-center justify-center p-4 sm:p-6 relative text-[#FAFAFA]">
      ...
      >
        <div className="flex flex-col items-center text-center mb-8">
          <img src="/favicon.png" alt="HireFlow Logo" className="w-12 h-12 mb-5 rounded-xl shadow-[0_0_15px_rgba(13,148,136,0.3)] mx-auto" />
          <h1 className="text-2xl sm:text-3xl font-bold mb-2 tracking-tight">Choose Your Access</h1>
          <p className="text-[#A1A1AA] text-sm">Continue as a Candidate or Administrator</p>
        </div>
          ...
          {(["candidate", "admin"] as const).map((tab) => (
            <button
              ...
              className={`flex-1 py-2.5 text-sm font-medium rounded-lg relative z-10 transition-colors ${
                activeTab === tab ? "text-white" : "text-[#A1A1AA] hover:text-[#FAFAFA]"
              }`}
            >
            ...
            {activeTab === "candidate" ? (
              ...
              >
                ...
                <p className="text-sm text-[#A1A1AA] mb-8 leading-relaxed">
                  Practice interviews with AI and receive detailed performance insights.
                </p>
              ...
              <motion.div
                ...
                className="flex flex-col h-full"
              >
                <h2 className="text-lg font-semibold mb-2">Administrator Access</h2>
                <p className="text-sm text-[#A1A1AA] mb-6">
                  Manage interviews, analytics and candidate evaluations.
                </p>

                <form onSubmit={(e) => { e.preventDefault(); handleLogin("/admin/dashboard"); }} className="space-y-4">
                  <div>
                    <input 
                      type="email" 
                      required
                  ...
                  <div className="flex items-center justify-between text-xs mt-2">
                    <label className="flex items-center gap-2 cursor-pointer group">
                      <input type="checkbox" className="w-3.5 h-3.5 rounded border-white/[0.1] bg-[#09090B] text-[#0D9488] focus:ring-[#0D9488]/50 focus:ring-offset-0" />
                      <span className="text-[#A1A1AA] group-hover:text-[#FAFAFA] transition-colors">Remember me</span>
                    </label>
                    <button type="button" className="text-[#0D9488] hover:text-[#06B6D4] transition-colors font-medium">
                      Forgot Password?
                    </button>
                  </div>
